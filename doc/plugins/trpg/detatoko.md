...
ただし後方互換のため、しばらくの期間は _.dlbp_ コマンドも今まで通り利用できるようにします。
...
クラスを選択します。  
基本ルールブックに掲載された12種のほか、サプリメント『でたとこフロンティア』で追加された3種のクラスも加えた全15種のクラスから1つをランダムに出力します。
...
```
.dpp
> でたとこサーガ [koi-chan]<PCポジション>: 凡人 (フロンティアp.43)
。でポジション
> でたとこサーガ [koi-chan]<PCポジション>: カリスマ (フロンティアp.49)
```
...
```
.dnpp
> でたとこサーガ [koi-chan]<敵NPCポジション>: 黒幕 (フロンティアp.71)
。で敵ポジション
> でたとこサーガ [koi-chan]<敵NPCポジション>: 侵略者 (フロンティアp.73)
```
...
```
.d1lcs 1
> [koi-chan]<1行キャラクターシート>: 1件ヒットしました
> 王道勇者    |１|11/11 10/10 0|勇者戦士|聖希折鉄強必|１１　４　　|   1|でたとこサーガ
> [koi-chan]<1行キャラクターシート>: 出力は以上です
.d1lcs title 1 2
> [koi-chan]<1行キャラクターシート>: 2件ヒットしました
